...
// import { fs, getResolvedPath, setFile } from '../util/filesystem';
...
import { dirname, join } from '../deno/path/posix';
 ...
 * @param logger Console log
 */
export const HTTP_RESOLVE = (assets: OutputFile[] = [], host = DEFAULT_CDN_HOST, logger = console.log) => {
  return async (args: OnResolveArgs): Promise<OnResolveResult> => {
    ...
    if (argPath[0] !== ".") {
      ...
      if (/^https?:\/\//.test(argPath)) {
        return {
          ...
          pluginData: { ...args.pluginData, pkg: args.pluginData?.pkg },
        };
      }
      ...
      if (isBareImport(argPath)) {
        return await CDN_RESOLVE(assets, origin, logger)(args);
      } else {
        /** 
    ...
    const filePath = resolveDir && args.importer ? join(args.importer, "../", argPath) : undefined;
    const path = urlJoin(args.pluginData?.url, "../", argPath);

    // Some typescript files don't have file extensions but you can't fetch a file without their file extension
    // so bundle tries to solve for that
    let urlStr: string;
    ({ url: urlStr } = await determineExtension(path, true));

    return {
      // ...args,
      path: resolveDir ? filePath : path,
      namespace: HTTP_NAMESPACE,
      pluginData: { ...args.pluginData, filePath, urlStr, pkg: args.pluginData?.pkg },
    };
  };
};
...
export const HTTP = (assets: OutputFile[] = [], host = DEFAULT_CDN_HOST, logger = console.log): Plugin => {
  return {
    ...
    setup(build) {
      ...
      build.onResolve({ filter: /.*/, namespace: HTTP_NAMESPACE }, HTTP_RESOLVE(assets, host, logger));
      ...
      build.onLoad({ filter: /.*/, namespace: HTTP_NAMESPACE }, async (args) => {
        // Some typescript files don't have file extensions but you can't fetch a file without their file extension
        // so bundle tries to solve for that
        let content: Uint8Array | undefined, url: string, ext: string;
        ({ content, url, ext } = await determineExtension(args.pluginData.urlStr ? args.pluginData.urlStr : args.path, false));
        ...
        if (content) {
          ...
          return {
            ...
            resolveDir: filePath ? dirname(filePath) : undefined,
