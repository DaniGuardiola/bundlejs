import type { OnResolveArgs, OnResolveResult, Plugin, OutputFile } from 'esbuild';

import { determineExtension, fetchAssets, fetchPkg, HTTP_NAMESPACE } from './http';
import { extname, isBareImport, join, urlJoin } from '../util/path';
...
import { root, setFileSystem } from '../util/filesystem';
 ...
 */
export const CDN_RESOLVE = (assets: OutputFile[] = [], cdn = DEFAULT_CDN_HOST, logger = console.log, rootPkg: Partial<PackageJson> = {}, preferVirtualFS = false) => {
  return async (args: OnResolveArgs): Promise<OnResolveResult | undefined> => {
    if (isBareImport(args.path)) {
        ...
        try {
          ...
          for (let i = 0; i < pkgVariantsLen; i++) {
            ...
            try {
              ...
              // setFile("/node_modules/" + pkgMetadata.path, JSON.stringify(pkg));
              const pkgPath = "/node_modules" + new URL(res.url).pathname.replace("@" + pkg.version, "");
              console.log("package.json ", pkgPath, res.url)
              try {
                setFileSystem({
      ...
      ({ content, url: urlStr, ext } = await determineExtension(url.toString(), !preferVirtualFS)); //
      ...
      if (content) { // 
        ...
        // const _assetResults =
        //     (await fetchAssets(urlStr, content, args.namespace, logger))
        //         .filter((result) => {
 ...
 * @param logger Console log
 */
export const CDN = (assets: OutputFile[] = [], cdn: string, pkgJSON: Partial<PackageJson> = {}, logger = console.log, RESOLVED_URLs = new Map<string, string>()): Plugin => {
  return {
    name: CDN_NAMESPACE,
    setup(build) {      
      build.onResolve({ filter: /.*/ }, async (args) => {
        ...
        if (isBareImport(args.path)) {
          if (RESOLVED_URLs.has(argPath)) return { path: RESOLVED_URLs.get(argPath) }
          ...
          let pkg: PackageJson = args.pluginData?.pkg ?? { ...pkgJSON };
          let oldPkg = pkg;
          ...
          if (RESOLVED_URLs.has(path)) return { path: RESOLVED_URLs.get(path) }
