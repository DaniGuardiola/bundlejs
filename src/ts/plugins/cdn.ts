...
import { setFileSystem } from '../util/filesystem';
          ...
          for (let i = 0; i < pkgVariantsLen; i++) {
            ...
            try {
              ...
              try {
                ...
                console.warn("Error: saving " + pkgPath, e)
              }
              break;
            } catch (e) {
 ...
 * @param logger Console log
 */
export const CDN = (assets: OutputFile[] = [], cdn: string, pkgJSON: Partial<PackageJson> = {}, logger = console.log, preferVirtualFS?: boolean): Plugin => {
  return {
    name: CDN_NAMESPACE,
    setup(build) {
      // Resolve bare imports to the CDN required using different URL schemes
      build.onResolve({ filter: /.*/ }, CDN_RESOLVE(assets, cdn, logger, pkgJSON, preferVirtualFS));
      build.onResolve({ filter: /.*/, namespace: CDN_NAMESPACE }, CDN_RESOLVE(assets, cdn, logger, pkgJSON, preferVirtualFS));
    },
  };
};
