...
import ESBUILD_WASM from "../../core/src/wasm.ts";

import { createNotice } from "../../core/src/utils/create-notice";
import { parseShareURLQuery, parseConfig } from "./_parse-query";
...
import styleText from "./_style.ts";
...
export default async function handler(req: Request) {
	...
	try {
		...
		const { content: _content, ...size } = await compress(result.contents.map((x: { contents: Uint8Array; }) => x.contents), configObj.compression);
